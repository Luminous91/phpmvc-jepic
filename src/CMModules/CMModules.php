<?php
/**
 * A model for managing Jepic modules.
 * 
 * @package JepicCore
 */
class CMModules extends CObject {
    ...
    while (($module = $dir->read()) !== false) {
      if(is_dir("$src/$module")) {
        if(class_exists($module)) {
          ...
          $modules[$module]['isJepicCore']   = in_array($rc->name, array('CJepic', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject'));
          $modules[$module]['isJepicCMF']    = in_array($rc->name, array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier'));
        }
      }
    }
    $dir->close();
  ...
  private function GetDetailsOfModule($module) {
    ...
    if(class_exists($module)) {
      ...
      $details['isJepicCore']   = in_array($rc->name, $this->jepicCoreModules);
      $details['isJepicCMF']    = in_array($rc->name, $this->jepicCMFModules);
